 ...
 */
import tailwindCss, { type Config } from "npm:tailwindcss@^3.4.4";
import postcss, {
  type Plugin,
  type ProcessOptions,
  type Processor,
  type Result,
} from "npm:postcss@^8.4.38";
import autoprefixer, {
  type Options as ConfigAutoprefixer,
} from "npm:autoprefixer@^10.4.19";
import cssnano, { type Options as ConfigCssnano } from "npm:cssnano@^7.0.2";
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname } from "node:path";
import type { Handler, NHttp } from "./deps.ts";
